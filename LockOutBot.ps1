if (-not(Test-Path -Path ".\API.json" -PathType Leaf)) {

    $Settings = [PSCustomObject]@{
        BotToken = ''
        AllowedID = @()
    }
    Write-Host "Welcome to LockOut Bot"
    $Settings.BotToken = Read-Host  "Bot Token"
    function AllowedID{
    $Settings.AllowedID += (Read-Host "Allowed ID")
    $confirmation = Read-Host "Add More?[Y/N]"
    if ($confirmation -eq 'y') {AllowedID}}
    AllowedID
    $Settings | ConvertTo-Json | Out-File ".\API.json"
    }
    $Settings=Get-Content -Path API.json | ConvertFrom-Json
    ...
    clear-host
    $TelegramApiUri = "https://api.telegram.org/bot$($Settings.BotToken)"
    $LockList = New-Object System.Collections.Generic.List[System.Object]
    
    Function Send-TelegramMessage {
        param([string]$Text) 
        $Uri = $TelegramApiUri + "/sendMessage"
        $obj = @{chat_id = ([string]$Settings.AllowedID); text = [string]$text } | ConvertTo-Json
    ...
    function Get-TelegramMessages {
        ...
        #Checks for your ID
        if ($Settings.AllowedID -notin "$AllowedID") {
    ...
    foreach ($x in (Search-ADAccount -LockedOut).SamAccountName){
        ...
        Get-TelegramMessages -AllowedID $Settings.AllowedID -TelegramApiUri $TelegramApiUri #-ErrorAction Ignore #read new commands from telegram
    }Until($false)